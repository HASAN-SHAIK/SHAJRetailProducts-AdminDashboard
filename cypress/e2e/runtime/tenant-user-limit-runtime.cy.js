...
const tenant = {
  id: Number(tenantId),
  shop_name: 'Cycle A User Limit Store',
...
const users = [
  ...
  },
  {
    ...
    email: 'cashier.limit@example.com',
    role: 'staff',
    all_branch_access: true
  }
];
...
describe('Admin runtime - tenant user plan limit reached', () => {
  it('disables registration and never issues a create request when authoritative user count is at max_users', () => {
    let tenantReads = 0;
    let userReads = 0;
    let branchReads = 0;
    let userCreates = 0;
    ...
    cy.intercept('GET', `**/tenant/${tenantId}`, (req) => {
      tenantReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-user-limit-token');
    ...
    cy.intercept('POST', `**/tenants/${tenantId}/users`, (req) => {
      userCreates += 1;
      req.reply({ statusCode: 500, body: { message: 'Registration must not be attempted at the plan limit' } });
    }).as('createUser');

    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-user-limit-token');
    ...
    cy.get('input[value="Cycle A User Limit Store"]').should('exist');
    ...
    cy.contains('h6', 'Tenant Users')
      ...
      .within(() => {
        ...
        cy.contains('admin.limit@example.com').should('be.visible');
        cy.contains('cashier.limit@example.com').should('be.visible');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-user-limit-token');
