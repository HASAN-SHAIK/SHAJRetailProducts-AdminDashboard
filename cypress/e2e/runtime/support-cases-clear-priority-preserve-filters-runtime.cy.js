describe('Cycle A Support Cases clear Priority preserving other filters runtime', () => {
  const sessionMarker = 'cycle-a-clear-priority-session';
  const requests = [];
  const cases = [
    { id: 1501, tenant_name: 'Cycle A Supermarket', title: 'Open urgent supermarket printer hardware issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T03:10:00Z', updated_at: '2026-09-11T03:20:00Z' },
    { id: 1502, tenant_name: 'Cycle A Supermarket', title: 'Open medium supermarket printer hardware issue', category: 'hardware', priority: 'medium', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T03:00:00Z', updated_at: '2026-09-11T03:10:00Z' },
    { id: 1503, tenant_name: 'Cycle A Pharmacy', title: 'Open urgent pharmacy scanner hardware issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T02:50:00Z', updated_at: '2026-09-11T03:00:00Z' },
    { id: 1504, tenant_name: 'Cycle A Supermarket', title: 'Open urgent supermarket reports issue', category: 'reports', priority: 'urgent', status: 'open', assigned_to: 'admin@example.com', created_at: '2026-09-11T02:40:00Z', updated_at: '2026-09-11T02:50:00Z' },
    { id: 1505, tenant_name: 'Cycle A Supermarket', title: 'Resolved urgent supermarket printer hardware issue', category: 'hardware', priority: 'urgent', status: 'resolved', assigned_to: 'admin@example.com', created_at: '2026-09-11T02:30:00Z', updated_at: '2026-09-11T02:40:00Z' }
  ];
  ...
  beforeEach(() => {
    ...
    cy.intercept({ method: 'GET', pathname: '/support/cases' }, (req) => {
      ...
      if (req.query.status === 'open' && req.query.category === 'hardware' && req.query.tenant === 'Cycle A Supermarket' && req.query.priority === undefined) {
  ...
  it('clears only Priority while preserving Status, Category and Tenant and restores the authoritative three-filter result set', () => {
    ...
    cy.then(() => {
      ...
      expect(requests[5].priority).to.be.undefined;
