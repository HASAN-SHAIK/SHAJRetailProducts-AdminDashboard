...
const unlimitedBranch = {
  ...existingBranch,
  max_devices_allowed: null,
  updated_at: '2026-09-11T13:50:00Z'
};

describe('Admin runtime - tenant branch unlimited device limit', () => {
  it('clears a finite device limit and renders the authoritative branch as Unlimited without refetching', () => {
    let branchReads = 0;
    let branchUpdates = 0;
    ...
    cy.intercept('GET', `**/tenants/${tenantId}/branches`, (req) => {
      branchReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-branch-unlimited-token');
      req.reply({ statusCode: 200, body: { branches: [existingBranch] } });
    }).as('branches');
    cy.intercept('PATCH', `**/tenants/${tenantId}/branches/${existingBranch.id}`, (req) => {
      branchUpdates += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-branch-unlimited-token');
      expect(req.body).to.deep.equal({ max_devices_allowed: null });
      req.reply({ statusCode: 200, body: { branch: unlimitedBranch } });
    }).as('updateBranch');

    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-branch-unlimited-token');
    ...
    cy.contains('tr', 'Main Branch').as('branchRow').should('contain.text', '2');
    ...
    cy.get('label').contains(/^Max Devices Allowed$/).invoke('attr', 'for').then((id) => {
      cy.get(`#${CSS.escape(id)}`).should('have.value', '2').clear().should('have.value', '');
    });
    ...
    cy.contains('tr', 'Main Branch').should('contain.text', 'Unlimited').and('contain.text', 'Hyderabad');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-branch-unlimited-token');
