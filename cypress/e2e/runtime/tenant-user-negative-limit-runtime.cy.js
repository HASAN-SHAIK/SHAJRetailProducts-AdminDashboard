...
const tenant = {
  id: Number(tenantId),
  shop_name: 'Cycle A Negative Limit Store',
...
const users = [
  { id: 101, name: 'Cycle A Existing User', email: 'existing.negative@example.com', role: 'admin', all_branch_access: true }
];
...
describe('Admin runtime - negative tenant user capacity metadata', () => {
  it('fails closed and does not expose registration when authoritative max_users is negative', () => {
    let tenantReads = 0;
    let userReads = 0;
    let branchReads = 0;
    let createPosts = 0;
    ...
    cy.intercept('GET', `**/tenant/${tenantId}`, (req) => {
      tenantReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-negative-limit-token');
    ...
    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-negative-limit-token');
    ...
    cy.get('input[value="Cycle A Negative Limit Store"]').should('exist');

    cy.contains('h6', 'Tenant Users')
      .should('be.visible')
      .closest('.MuiCardContent-root')
      .within(() => {
        cy.contains('existing.negative@example.com').should('be.visible');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-negative-limit-token');
