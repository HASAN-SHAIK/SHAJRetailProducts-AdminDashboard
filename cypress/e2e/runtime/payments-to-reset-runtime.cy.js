describe('Cycle A Payments To reset runtime', () => {
  const sessionValue = 'cycle-a-payments-to-reset-token';
  let paymentsReads = 0;
    ...
    cy.intercept('GET', '**/subscription-payments*', (req) => {
      ...
      if (paymentsReads === 1) {
        ...
        req.reply({ statusCode: 200, body: { data: { payments: [
          { id: 991, shop_name: 'Cycle A Early To Reset Shop', plan_name: 'Basic', amount: '499.00', payment_method: 'UPI', paid_at: '2026-09-03T10:00:00.000Z', status: 'paid' },
          { id: 992, shop_name: 'Cycle A Late To Reset Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'CARD', paid_at: '2026-09-08T10:00:00.000Z', status: 'paid' }
        ] } } });
        return;
      }
  ...
  it('removes To from the request and restores authoritative unfiltered rows when cleared', () => {
    ...
    cy.contains('Cycle A Early To Reset Shop').should('be.visible');
    cy.contains('Cycle A Late To Reset Shop').should('be.visible');

    cy.get('input[type="date"]').eq(1).type('2026-09-04');
    cy.wait('@paymentsBoundary').its('response.statusCode').should('eq', 200);
    cy.get('input[type="date"]').eq(1).should('have.value', '2026-09-04');
    cy.contains('Cycle A Early To Reset Shop').should('be.visible');
    cy.contains('Cycle A Late To Reset Shop').should('not.exist');
