describe('Cycle A Payments Plan + From composition runtime', () => {
  const sessionValue = 'cycle-a-payments-plan-from-token';
  let paymentsReads = 0;
    ...
    cy.intercept('GET', '**/subscription-payments*', (req) => {
      ...
      if (paymentsReads === 1) {
        ...
        req.reply({ statusCode: 200, body: { data: { payments: [
          { id: 931, shop_name: 'Cycle A Old Pro Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'CARD', paid_at: '2026-08-30T10:00:00.000Z', status: 'paid' },
          { id: 932, shop_name: 'Cycle A September Basic Shop', plan_name: 'Basic', amount: '499.00', payment_method: 'UPI', paid_at: '2026-09-02T10:00:00.000Z', status: 'paid' },
          { id: 933, shop_name: 'Cycle A September Pro Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'UPI', paid_at: '2026-09-03T10:00:00.000Z', status: 'paid' }
        ] } } });
        return;
      }
  ...
  it('preserves the selected Plan while adding From and converges to the authoritative intersection', () => {
    ...
    cy.contains('Cycle A Old Pro Shop').should('be.visible');
    ...
    cy.contains('Cycle A September Pro Shop').should('be.visible');
    cy.contains('Cycle A Old Pro Shop').should('not.exist');
    cy.contains('Cycle A September Basic Shop').should('not.exist');
