describe('Cycle A Support Cases priority + tenant composition runtime', () => {
  const token = 'cycle-a-support-priority-tenant-token';
  const requests = [];
  const cases = [
    { id: 1271, tenant_name: 'Cycle A Supermarket', title: 'Urgent supermarket printer issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-10T11:00:00Z', updated_at: '2026-09-10T11:10:00Z' },
    { id: 1272, tenant_name: 'Cycle A Pharmacy', title: 'Urgent pharmacy scanner issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-10T10:00:00Z', updated_at: '2026-09-10T10:10:00Z' },
    { id: 1273, tenant_name: 'Cycle A Supermarket', title: 'Medium supermarket report issue', category: 'reports', priority: 'medium', status: 'open', assigned_to: 'admin@example.com', created_at: '2026-09-10T09:00:00Z', updated_at: '2026-09-10T09:10:00Z' }
  ];
  ...
  beforeEach(() => {
    ...
    cy.intercept({ method: 'GET', pathname: '/support/cases' }, (req) => {
      ...
      if (req.query.priority === 'urgent' && req.query.tenant === 'Cycle A Supermarket') {
        return req.reply({ statusCode: 200, body: page([cases[0]]) });
      }
      if (req.query.priority === 'urgent') {
  ...
  it('preserves priority while adding tenant and converges to the authoritative intersection', () => {
    ...
    cy.contains('Urgent supermarket printer issue').should('be.visible');
    cy.contains('Urgent pharmacy scanner issue').should('be.visible');
    cy.contains('Medium supermarket report issue').should('be.visible');
    ...
    cy.contains('Urgent supermarket printer issue').should('be.visible');
    cy.contains('Urgent pharmacy scanner issue').should('be.visible');
    cy.contains('Medium supermarket report issue').should('not.exist');
    ...
    cy.contains('Urgent supermarket printer issue').should('be.visible');
    cy.contains('Urgent pharmacy scanner issue').should('not.exist');
    cy.contains('Medium supermarket report issue').should('not.exist');
    ...
    cy.then(() => {
      ...
      expect(requests[1].priority).to.eq('urgent');
