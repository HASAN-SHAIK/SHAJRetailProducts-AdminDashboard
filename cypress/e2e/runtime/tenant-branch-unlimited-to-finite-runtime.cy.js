...
describe('Admin runtime - tenant branch unlimited to finite device limit', () => {
  it('sets an Unlimited branch to a finite device limit and renders the authoritative result without refetching', () => {
    let branchReads = 0;
    let branchUpdates = 0;
    ...
    cy.intercept('GET', `**/tenants/${tenantId}/branches`, (req) => {
      branchReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-branch-finite-token');
    ...
    cy.intercept('PATCH', `**/tenants/${tenantId}/branches/${existingBranch.id}`, (req) => {
      ...
      req.reply({ statusCode: 200, body: { branch: finiteBranch } });
    }).as('updateBranch');

    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-branch-finite-token');
    ...
    cy.contains('tr', 'Main Branch').as('branchRow').should('contain.text', 'Unlimited').and('contain.text', 'Hyderabad');
    ...
    cy.get('label').contains(/^Max Devices Allowed$/).invoke('attr', 'for').then((id) => {
      cy.get(`#${CSS.escape(id)}`).should('have.value', '').type('5').should('have.value', '5');
    });
    ...
    cy.contains('tr', 'Main Branch').should('contain.text', '5').and('contain.text', 'Hyderabad').and('not.contain.text', 'Unlimited');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-branch-finite-token');
