describe('Cycle A Payments Plan reset runtime', () => {
  const sessionValue = 'cycle-a-payments-plan-reset-token';
  let paymentsReads = 0;
    ...
    cy.intercept('GET', '**/subscription-payments*', (req) => {
      ...
      if (paymentsReads === 1) {
        ...
        req.reply({ statusCode: 200, body: { data: { payments: [
          { id: 961, shop_name: 'Cycle A Reset Pro Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'CARD', paid_at: '2026-09-03T10:00:00.000Z', status: 'paid' },
          { id: 962, shop_name: 'Cycle A Reset Basic Shop', plan_name: 'Basic', amount: '499.00', payment_method: 'UPI', paid_at: '2026-09-03T11:00:00.000Z', status: 'paid' }
        ] } } });
        return;
      }
  ...
  it('removes plan from the request and restores authoritative unfiltered rows when reset to All', () => {
    ...
    cy.contains('Cycle A Reset Pro Shop').should('be.visible');
    cy.contains('Cycle A Reset Basic Shop').should('be.visible');

    cy.get('[role="combobox"]').last().click();
    cy.contains('[role="option"]', 'Pro').click();
    cy.wait('@paymentsBoundary').its('response.statusCode').should('eq', 200);
    cy.contains('Cycle A Reset Pro Shop').should('be.visible');
    cy.contains('Cycle A Reset Basic Shop').should('not.exist');
