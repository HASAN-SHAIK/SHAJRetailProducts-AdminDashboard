describe('Cycle A Payments From reset runtime', () => {
  const sessionValue = 'cycle-a-payments-from-reset-token';
  let paymentsReads = 0;
    ...
    cy.intercept('GET', '**/subscription-payments*', (req) => {
      ...
      if (paymentsReads === 1) {
        ...
        req.reply({ statusCode: 200, body: { data: { payments: [
          { id: 981, shop_name: 'Cycle A August Reset Shop', plan_name: 'Basic', amount: '499.00', payment_method: 'UPI', paid_at: '2026-08-29T10:00:00.000Z', status: 'paid' },
          { id: 982, shop_name: 'Cycle A September Reset Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'CARD', paid_at: '2026-09-03T10:00:00.000Z', status: 'paid' }
        ] } } });
        return;
      }
  ...
  it('removes From from the request and restores authoritative unfiltered rows when cleared', () => {
    ...
    cy.contains('Cycle A August Reset Shop').should('be.visible');
    cy.contains('Cycle A September Reset Shop').should('be.visible');

    cy.get('input[type="date"]').eq(0).type('2026-09-01');
    cy.wait('@paymentsBoundary').its('response.statusCode').should('eq', 200);
    cy.get('input[type="date"]').eq(0).should('have.value', '2026-09-01');
    cy.contains('Cycle A August Reset Shop').should('not.exist');
    cy.contains('Cycle A September Reset Shop').should('be.visible');
