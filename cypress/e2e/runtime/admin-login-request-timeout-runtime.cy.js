describe('V1 Admin login request-timeout runtime', () => {
  it('surfaces authoritative 408 feedback, preserves retry state, and remains unauthenticated', () => {
    let loginRequests = 0;
    ...
    cy.get('input[name="email"]').type('cycle-a-408@example.com');
    cy.get('input[name="password"]').type('retry-password');
    cy.get('button[type="submit"]').click();
    cy.wait('@login408').its('response.statusCode').should('eq', 408);

    cy.location('pathname').should('eq', '/admin/login');
    cy.contains('Request timeout').should('be.visible');
    cy.get('input[name="email"]').should('have.value', 'cycle-a-408@example.com');
