describe('Cycle A Support Cases priority success runtime', () => {
  const token = 'cycle-a-support-priority-success-token';
  const requests = [];

  const unfiltered = {
    data: {
      cases: [
        { id: 1201, tenant_name: 'Cycle A Market', title: 'Printer unavailable', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-10T06:00:00Z', updated_at: '2026-09-10T06:10:00Z' },
        { id: 1202, tenant_name: 'Cycle A Pharmacy', title: 'Report discrepancy', category: 'reports', priority: 'medium', status: 'open', assigned_to: 'admin@example.com', created_at: '2026-09-10T05:00:00Z', updated_at: '2026-09-10T05:15:00Z' }
      ],
      total: 2,
      page: 1,
      pageSize: 10
  ...
  beforeEach(() => {
    ...
    cy.intercept({ method: 'GET', pathname: '/support/cases' }, (req) => {
      ...
      if (req.query.priority === 'urgent') {
        ...
        req.reply({ statusCode: 200, body: urgentOnly });
        return;
      }
  ...
  it('refetches with only priority=urgent and converges to authoritative rows while retaining session and route', () => {
