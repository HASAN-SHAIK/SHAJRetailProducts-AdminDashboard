describe("V1 Admin activity logs filter failure recovery runtime", () => {
  it("recovers authoritative unfiltered rows after resetting a failed Action filter to All", () => {
    const token = "cycle-a-logs-filter-failure-recovery-token";
    let requestCount = 0;
    ...
    cy.intercept("GET", "**/activity-logs*", (req) => {
            ...
            data: {
              logs: [
                ...
                },
                {
                  ...
                  entity_id: 42,
                  metadata: { source: "initial-authoritative" },
                  created_at: "2026-09-09T03:01:00.000Z"
                }
              ]
            }
          }
      ...
      if (requestCount === 2) {
        ...
        req.reply({
          statusCode: 500,
          body: { message: "Activity log filter service unavailable" }
        });
        return;
      }
    ...
    cy.contains("1801").should("be.visible");
    cy.contains("1802").should("be.visible");
    ...
    cy.contains("Activity log filter service unavailable").should("be.visible");
    cy.contains("1801").should("not.exist");
    cy.contains("1802").should("not.exist");
    ...
    cy.contains("Activity log filter service unavailable").should("not.exist");
