describe("V1 Admin activity logs filter failure runtime", () => {
  it("surfaces a failed Action filter refetch without presenting stale unfiltered rows as filtered results", () => {
    const token = "cycle-a-logs-filter-failure-token";
    let requestCount = 0;
            ...
            data: {
              logs: [
                ...
                },
                {
                  ...
                  entity_id: 42,
                  metadata: { source: "initial-authoritative" },
                  created_at: "2026-09-08T15:01:00.000Z"
                }
              ]
            }
          }
    ...
    cy.contains("1502").should("be.visible");
    ...
    cy.contains("1501").should("not.exist");
    cy.contains("1502").should("not.exist");
