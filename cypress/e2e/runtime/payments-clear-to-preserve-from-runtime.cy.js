describe('Cycle A Payments clear To preserves From runtime', () => {
  const sessionValue = 'cycle-a-payments-clear-to-preserve-from-token';
  let paymentsReads = 0;
    ...
    cy.intercept('GET', '**/subscription-payments*', (req) => {
      ...
      if (paymentsReads === 1) {
        ...
        req.reply({ statusCode: 200, body: { data: { payments: [
          { id: 1011, shop_name: 'Cycle A Before From Shop', plan_name: 'Basic', amount: '499.00', payment_method: 'UPI', paid_at: '2026-08-28T10:00:00.000Z', status: 'paid' },
          { id: 1012, shop_name: 'Cycle A Inside Clear To Shop', plan_name: 'Pro', amount: '999.00', payment_method: 'CARD', paid_at: '2026-09-03T10:00:00.000Z', status: 'paid' },
          { id: 1013, shop_name: 'Cycle A After To Restore Shop', plan_name: 'Premium', amount: '1999.00', payment_method: 'UPI', paid_at: '2026-09-08T10:00:00.000Z', status: 'paid' }
        ] } } });
        return;
      }
  ...
  it('clears To, preserves From, and converges to authoritative From-only rows', () => {
    ...
    cy.contains('Cycle A Before From Shop').should('be.visible');
    cy.contains('Cycle A Inside Clear To Shop').should('be.visible');
    cy.contains('Cycle A After To Restore Shop').should('be.visible');

    cy.get('input[type="date"]').eq(0).type('2026-09-01');
    cy.wait('@paymentsBoundary').its('response.statusCode').should('eq', 200);
    cy.contains('Cycle A Before From Shop').should('not.exist');
    cy.contains('Cycle A Inside Clear To Shop').should('be.visible');
    cy.contains('Cycle A After To Restore Shop').should('be.visible');
    ...
    cy.contains('Cycle A Before From Shop').should('not.exist');
    cy.contains('Cycle A Inside Clear To Shop').should('be.visible');
    cy.contains('Cycle A After To Restore Shop').should('not.exist');
