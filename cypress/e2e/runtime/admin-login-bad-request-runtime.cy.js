describe('V1 Admin login bad-request runtime', () => {
  it('surfaces authoritative 400 feedback, preserves retry state, and remains unauthenticated', () => {
    let loginRequests = 0;
    cy.intercept('POST', '**/auth/login', (req) => {
      loginRequests += 1;
      req.reply({ statusCode: 400, body: { message: 'Login request is invalid' } });
    }).as('login400');
    ...
    cy.get('input[name="email"]').type('cycle-a-400@example.com');
    cy.get('input[name="password"]').type('retry-password');
    cy.get('button[type="submit"]').click();
    cy.wait('@login400').its('response.statusCode').should('eq', 400);

    cy.location('pathname').should('eq', '/admin/login');
    cy.contains('Login request is invalid').should('be.visible');
    cy.get('input[name="email"]').should('have.value', 'cycle-a-400@example.com');
