...
const tenant = {
  id: Number(tenantId),
  shop_name: 'Cycle A Zero Limit User Store',
  ...
  plan_features: { max_users: 0 },
  subscription: { plan: 'premium' },
  addons: {}
};

const users = [
  { id: 101, name: 'Cycle A Admin', email: 'admin.zero@example.com', role: 'admin', all_branch_access: true },
  { id: 102, name: 'Cycle A Cashier', email: 'cashier.zero@example.com', role: 'staff', all_branch_access: true }
];
...
describe('Admin runtime - tenant user zero limit semantics', () => {
  it('keeps registration available when authoritative max_users is zero because zero is non-finite-capacity semantics', () => {
    let tenantReads = 0;
    let userReads = 0;
    let branchReads = 0;
    let createPosts = 0;
    ...
    cy.intercept('GET', `**/tenant/${tenantId}`, (req) => {
      tenantReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-user-zero-limit-token');
    ...
    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-user-zero-limit-token');
    ...
    cy.get('input[value="Cycle A Zero Limit User Store"]').should('exist');
    ...
    cy.contains('h6', 'Tenant Users')
      ...
      .within(() => {
        ...
        cy.contains('admin.zero@example.com').should('be.visible');
        cy.contains('cashier.zero@example.com').should('be.visible');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-user-zero-limit-token');
