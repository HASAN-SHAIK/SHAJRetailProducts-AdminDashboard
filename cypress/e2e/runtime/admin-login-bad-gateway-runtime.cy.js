describe('V1 Admin login bad-gateway runtime', () => {
  it('surfaces authoritative 502 feedback, preserves retry state, and remains unauthenticated', () => {
    let loginRequests = 0;
    cy.intercept('POST', '**/auth/login', (req) => {
      loginRequests += 1;
      req.reply({ statusCode: 502, body: { message: 'Bad gateway' } });
    }).as('login502');
    ...
    cy.get('input[name="email"]').type('cycle-a-502@example.com');
    cy.get('input[name="password"]').type('retry-password');
    cy.get('button[type="submit"]').click();
    cy.wait('@login502').its('response.statusCode').should('eq', 502);

    cy.location('pathname').should('eq', '/admin/login');
    cy.contains('Bad gateway').should('be.visible');
    cy.get('input[name="email"]').should('have.value', 'cycle-a-502@example.com');
