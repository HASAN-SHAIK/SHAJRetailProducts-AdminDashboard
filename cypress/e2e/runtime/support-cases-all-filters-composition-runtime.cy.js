describe('Cycle A Support Cases all-filter composition runtime', () => {
  const sessionMarker = 'cycle-a-all-filters-session';
  const requests = [];
  const cases = [
    { id: 1301, tenant_name: 'Cycle A Supermarket', title: 'Open urgent supermarket printer hardware issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T02:10:00Z', updated_at: '2026-09-11T02:20:00Z' },
    { id: 1302, tenant_name: 'Cycle A Supermarket', title: 'Open medium supermarket printer hardware issue', category: 'hardware', priority: 'medium', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T02:00:00Z', updated_at: '2026-09-11T02:10:00Z' },
    { id: 1303, tenant_name: 'Cycle A Pharmacy', title: 'Open urgent pharmacy scanner hardware issue', category: 'hardware', priority: 'urgent', status: 'open', assigned_to: 'ops@example.com', created_at: '2026-09-11T01:50:00Z', updated_at: '2026-09-11T02:00:00Z' },
    { id: 1304, tenant_name: 'Cycle A Supermarket', title: 'Open urgent supermarket reports issue', category: 'reports', priority: 'urgent', status: 'open', assigned_to: 'admin@example.com', created_at: '2026-09-11T01:40:00Z', updated_at: '2026-09-11T01:50:00Z' },
    { id: 1305, tenant_name: 'Cycle A Supermarket', title: 'Resolved urgent supermarket printer hardware issue', category: 'hardware', priority: 'urgent', status: 'resolved', assigned_to: 'admin@example.com', created_at: '2026-09-11T01:30:00Z', updated_at: '2026-09-11T01:40:00Z' }
  ];
  ...
  beforeEach(() => {
    ...
    cy.intercept({ method: 'GET', pathname: '/support/cases' }, (req) => {
      ...
      if (req.query.status === 'open' && req.query.category === 'hardware' && req.query.priority === 'urgent') {
        return req.reply({ statusCode: 200, body: page([cases[0], cases[2]]) });
      }
      if (req.query.status === 'open' && req.query.category === 'hardware') {
        return req.reply({ statusCode: 200, body: page([cases[0], cases[1], cases[2]]) });
      }
  ...
  it('preserves status, category and priority while adding tenant and converges to the authoritative four-filter intersection', () => {
    ...
    cases.forEach((item) => cy.contains(item.title).should('be.visible'));
    ...
    cy.contains(cases[2].title).should('be.visible');
    ...
    cy.then(() => {
      ...
      expect(requests[0].priority).to.be.undefined;
      ...
      expect(requests[3].priority).to.eq('urgent');
      ...
      expect(String(requests[4].page)).to.eq('1');
      expect(String(requests[4].pageSize)).to.eq('10');
    });
  });
});