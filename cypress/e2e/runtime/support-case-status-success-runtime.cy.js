describe('Cycle A Support Case status update success runtime', () => {
  const sessionValue = 'cycle-a-support-status-success-session';
  let detailReads = 0;
  let statusWrites = 0;
  ...
  beforeEach(() => {
    ...
    cy.intercept({ method: 'PATCH', pathname: '/support/cases/42/status' }, (req) => {
      ...
      req.reply({ statusCode: 200, body: { id: 42, status: 'resolved', updated_at: '2026-09-06T10:30:00.000Z' } });
    }).as('supportCaseStatusBoundary');
  });

  it('submits one authenticated status mutation and converges to authoritative returned state without a refetch', () => {
    ...
    cy.get('[role="dialog"]').within(() => { cy.contains('Change Status').should('be.visible'); cy.get('[role="combobox"]').click(); });
    cy.get('[role="option"]').contains('resolved').click();
    cy.get('[role="dialog"]').contains('button', 'Save').click();
    cy.wait('@supportCaseStatusBoundary').its('response.statusCode').should('eq', 200);
    ...
    cy.then(() => { expect(detailReads).to.eq(1); expect(statusWrites).to.eq(1); });
  });
});
