...
const tenant = {
  id: Number(tenantId),
  shop_name: 'Cycle A Over Limit Store',
...
const users = [
  {
    id: 81,
    name: 'Cycle A Admin',
    email: 'admin.overlimit@example.com',
    role: 'admin',
  ...
  },
  {
    ...
    email: 'cashier.overlimit@example.com',
    role: 'staff',
    all_branch_access: true
  }
];
...
describe('Admin runtime - tenant user count above plan limit', () => {
  it('fails closed when authoritative users already exceed max_users', () => {
    let tenantReads = 0;
    let userReads = 0;
    let branchReads = 0;
    let userCreates = 0;
    ...
    cy.intercept('GET', `**/tenant/${tenantId}`, (req) => {
      tenantReads += 1;
      expect(req.headers.authorization).to.equal('Bearer cycle-a-over-limit-token');
    ...
    cy.intercept('POST', `**/tenants/${tenantId}/users`, (req) => {
      userCreates += 1;
      req.reply({ statusCode: 500, body: { message: 'Registration must not be attempted above the plan limit' } });
    }).as('createUser');

    cy.visit(`/admin/tenants/${tenantId}`, {
      onBeforeLoad(win) {
        win.localStorage.setItem('shaj_admin_token', 'cycle-a-over-limit-token');
    ...
    cy.get('input[value="Cycle A Over Limit Store"]').should('exist');
    ...
    cy.contains('h6', 'Tenant Users')
      ...
      .within(() => {
        ...
        cy.contains('User limit reached (2/1). Upgrade the plan to add more users.').should('be.visible');
        cy.contains('admin.overlimit@example.com').should('be.visible');
        cy.contains('cashier.overlimit@example.com').should('be.visible');
    ...
    cy.window().then((win) => {
      expect(win.localStorage.getItem('shaj_admin_token')).to.equal('cycle-a-over-limit-token');
